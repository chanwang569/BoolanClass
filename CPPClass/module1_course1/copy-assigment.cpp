...
public:
    ...
    {
        if(this== &rect){ //处理相等的情况
            return *this;
        }

        this->m_length=rect.m_length;//非指针则直接复制
        this->m_width=rect.m_width;

        if(rect.m_leftUp!=nullptr){//指针则先判空。
            if(m_leftUp!=nullptr){
                *m_leftUp=*rect.m_leftUp;// Point assignment=  //注意。这里是修改指针指向的对象
            }
            else{
