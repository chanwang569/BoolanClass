// g++ -std=c++17 -pthread -I../common 26_threads_w_atomic.cpp
//
...
void sequential_increment(volatile int& n)
{
    auto t1 = rdtsc();
    {
        std::lock_guard guard{inc_lock};
...
int main()
{
    ...
    {
        std::cout << "*** Testing with two sequential_increment threads\n";
        std::thread t1{sequential_increment, std::ref(v1)};
        std::thread t2{sequential_increment, std::ref(v2)};
        t1.join();
        t2.join();
    }
}
